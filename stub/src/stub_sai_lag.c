    ...
    _In_ sai_attribute_t *attr_list)
{
    ...
    const sai_attribute_value_t *lag_id_val;
    uint32_t lag_id_idx;
    ...
    if (status != SAI_STATUS_SUCCESS) {
        printf("LAG ID attribute not found.\n");
        return status;
    }
    lag_db.members[lag_member_db_id].lag_oid = lag_id_val->oid;

    const sai_attribute_value_t *port_id_val;
    ...
    if (status != SAI_STATUS_SUCCESS) {
        printf("PORT ID attribute not found.\n");
        return status;
    }
    lag_db.members[lag_member_db_id].port_oid = port_id_val->oid;

    uint32_t lag_db_id;
    status = stub_object_to_type(lag_db.members[lag_member_db_id].lag_oid,
                                SAI_OBJECT_TYPE_LAG,
                                &lag_db_id);
    if (status != SAI_STATUS_SUCCESS) {
        printf("Invalid LAG ID in member\n");
        return status;
    }

    for (int i = 0; i < MAX_NUMBER_OF_LAG_MEMBERS; i++) {
        if (lag_db.lags[lag_db_id].members_ids[i] == 0) {
            lag_db.lags[lag_db_id].members_ids[i] = *lag_member_id;
            break;
        }
    }
    ...
    _In_ sai_object_id_t  lag_member_id)
{
    ...
    if (status != SAI_STATUS_SUCCESS) {
        printf("Invalid LAG OID in LAG_MEMBER\n");
        return status;
    }
...
sai_status_t stub_set_lag_member_attribute(
    ...
    _In_ const sai_attribute_t *attr)
{
    printf("SET LAG MEMBER ATTR: 0x%lX\n", lag_member_id);
    return SAI_STATUS_SUCCESS;
}
    ...
    _Inout_ sai_attribute_t *attr_list)
{
    ...
    printf("GET LAG MEMBER ATTR: 0x%lX\n", lag_member_id);
                               ...
                               void                          *arg)
{
    ...
    case SAI_LAG_ATTR_PORT_LIST:
        ...
        uint32_t ii = 0;
        for ( ; ii < MAX_NUMBER_OF_LAG_MEMBERS; ii++) {
            sai_object_id_t lag_member_oid = lag_db.lags[db_index].members_ids[ii];
            if (lag_member_oid == 0) continue;

            uint32_t lag_member_db_index;
            status = stub_object_to_type(lag_member_oid, SAI_OBJECT_TYPE_LAG_MEMBER, &lag_member_db_index);
            if (status == SAI_STATUS_SUCCESS) count++;
        }
        ...
        for ( ; ii < MAX_NUMBER_OF_LAG_MEMBERS; ii++) {
            ...
            value->objlist.list[value->objlist.count++] = lag_db.members[lag_member_db_index].port_oid;
        }
